<?php

use app\models\CustomHelper;
use app\models\Departments;
use app\models\Section;
use app\models\Sections;
use kartik\select2\Select2;
use kartik\widgets\ActiveForm;
use yii\helpers\Html;


/** @var yii\web\View $this */
/** @var app\models\Sections $model */
/** @var yii\widgets\ActiveForm $form */
?>

<div class="sections-form">

<?php
    if ($model->isNewRecord) {
        $action = 'create';
        $parent = '#rcamodal';
        $md = 4;
    } else {

        $action = 'update?rca=' . Yii::$app->getSecurity()->hashData($model->sid, 'gmtdev');
        $parent = '';
        $md = 3;
    }
    $form = ActiveForm::begin([
        'action' => [$action],
        'method' => 'post',

    ]); ?>


    







    <div class="row">
        <div class="col-md-6">
        <?= $form->field($model, 'name')->textInput(['maxlength' => true]) ?>

        </div>
        <div class="col-md-6">
        <?= $form->field($model, 'sname')->textInput(['maxlength' => true]) ?>
        </div>
    </div>



    <div class="row">
        <div class="col-md-6">
        <?= $form->field($model, 'did')->widget(Select2::class, [
                'data' => Section::getDepartmentList(),
                'options' => ['placeholder' => '--Select department--', 'required' => true],
                'pluginOptions' => [
                    'allowClear' => true,
                    'dropdownParent' => $parent
                ],
            ]) ?>
        </div>

        <div class="col-md-6">
        <?php if (!$model->isNewRecord) : ?>
                <?= $form->field($model, 'status')->dropDownList(CustomHelper::getStatusOptions()) ?>
        <?php endif; ?>
        </div>
    </div>

   



    <div class="form-group">
       
   

       <?php if ($model->isNewRecord) : ?>
                   <button type="button" class=" btn btn-outline-secondary" data-bs-dismiss="modal">Cancel</button>
                   <?php echo Html::submitButton(Yii::t('app', 'Save'), ['class' => 'btn btn-outline-info']); ?>
               <?php else : ?>
                <?= Html::a('<i class="fas fa-angle-double-left"></i>Back', ['index'], ['class' => 'btn btn-dark']) ?>
                   <?php echo Html::submitButton(Yii::t('app', 'Update'), ['class' => 'btn btn-primary']); ?>
               <?php endif; ?>
               </div>

    <?php ActiveForm::end(); ?>

</div>
